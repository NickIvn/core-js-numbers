...
function getSumOfDigits(num) {
  const digitArray = String(Math.abs(num)).split('');
  const sum = digitArray.reduce((acc, digit) => acc + parseInt(digit, 10), 0);
  return sum;
}
...
function getSine(num) {
  return Math.floor(Math.sinh(Math.floor(num)));
}
